...
function createPublishTask(db, product, body, asset, helpers) {
  ...
  if (!check.error && product.stock > 0) {
    if (product.publishStatus !== "已发布") product.publishStatus = "待发布";
    if (asset) {
      asset.status = "已加入发布";
      asset.usage = "发布任务";
      const kindField =
        asset.kind === "copy" ? "copyStatus" : asset.kind === "video" ? "videoStatus" : "imageStatus";
      if (product[kindField] === "已生成") product[kindField] = "已加入发布";
    }
...
function executePublishTask(db, taskId, helpers) {
  ...
  if (check.error) {
    ...
    if (platformResult.ok) {
      ...
      if (product) {
        product.publishStatus = "已发布";
        if (product.status !== "已暂停") product.status = "已发布";
      }
      if (asset) {
        asset.status = "已发布";
        asset.usage = "已发布";
  ...
  (db.publishTasks || []).forEach((task) => {
    ...
    if (task.status === "已成功") {
      ...
      if (product) {
        ...
        if (product.status !== "已发布" && product.status !== "已暂停") {
          product.status = "已发布";
          changed = true;
        }
      }
