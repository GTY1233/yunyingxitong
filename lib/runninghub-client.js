...
async function runRunningHubAiApp(appId, nodeInfoList, options = {}) {
  ...
  if (!results.length && payload.taskId) {
    results = await pollRunningHubTask(payload.taskId, options.label || "任务", options.onStage);
  }
  return { payload, results };
}

// onStage(status):QUEUED/RUNNING 状态变化时回调,供上层显示「排队中/生成中」。
async function pollRunningHubTask(taskId, label = "任务", onStage) {
  // 上限须覆盖「排队 + 运行」:生视频运行 20~40min,高峰期还要排队(QUEUED)。
  // 默认 1440×5s=120min 兜底(.env RUNNINGHUB_POLL_MAX 可再调大)。排队不算卡死,继续等。
  const maxAttempts = Number(process.env.RUNNINGHUB_POLL_MAX || 1440);
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
    ...
    // 记录状态变化(排队中 QUEUED → 运行中 RUNNING),便于诊断"到底是排队还是真卡";并回调前端
  ...
  const mins = Math.round((maxAttempts * interval) / 60000);
  const queued = lastStatus === "QUEUED";
  throw new Error(
    `RunningHub ${label}超时(等了约${mins}分钟,最后状态:${lastStatus || "未知"})` +
      (queued ? ";高峰期一直在排队未轮到,可稍后重试或调大 RUNNINGHUB_POLL_MAX" : "")
  );
}
