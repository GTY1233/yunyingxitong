...
function resolveLocalMediaPath(value, context = {}) {
  ...
  const matched = (context.productImages || []).find((item) => item.url === text || item.id === text);
  ...
  if (text.startsWith("/generated/")) {
    ...
    const filePath = path.join(context.rootDir || path.join(__dirname, ".."), "data", "generated", rel);
...
async function submitRunningHubAiApp(appId, nodeInfoList, options = {}) {
  ...
  if (payload.status === "FAILED") {
    const detail = payload.failedReason ? JSON.stringify(payload.failedReason) : (payload.errorMessage || "");
