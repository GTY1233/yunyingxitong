...
// 把语义参数覆盖到新视频工作流(app 1975951975441412098)的节点:
// 帧率→264、表情强度→265、胸部抖动幅度→266;秒数→加载帧上限422(= 秒 × 帧率,决定时长上限)。
// 分辨率走预设节点 470,不再传像素宽高;自定义比例(451/450)默认关闭,避免旧像素值污染比例。
function overrideVideoDefaults(nodeInfoList, options = {}) {
  const overrides = {
    264: valueOrDefault(options.frameRate, ""), // 帧率
    265: valueOrDefault(options.expressionIntensity, ""), // 表情强度
    266: valueOrDefault(options.ruKilnAmplitude, ""), // 胸部抖动幅度
  };
  const secs = Number(options.seconds);
  const fps = Number(options.frameRate) || 30;
  if (Number.isFinite(secs) && secs > 0) {
    overrides["422"] = String(Math.round(secs * fps)); // 加载帧上限 = 秒数 × 帧率
  }
