...
// 把视频工作流(app 1975951975441412098)全部可调项覆盖到对应节点。未传的保留工作流默认。
// 布尔项(开关)接受 true/false 或 "true"/"false";数值/档位直接透传。key→节点见下。
function overrideVideoDefaults(nodeInfoList, options = {}) {
  const KEY_TO_NODE = {
    zipMode: "535", // zip不支持cn站(cn站用1正常输出)
    poseCalcMode: "293", // 姿势计算方式
    poseLongNeck: "497", // 姿势3若脖子长开启(bool)
    poseStrength: "297", // 姿势强度
    cameraMove: "370", // 运镜开关(bool)
    cameraStrength: "361", // 运镜强度
    maskHelmet: "271", // 面具头盔模式(bool)
    expressionIntensity: "265", // 表情强度
    ruKilnAmplitude: "266", // 胸部抖动幅度
    skipFrames: "499", // 跳过前面多少帧
    maxFrames: "422", // 加载帧上限(决定时长)
    frameRate: "264", // 帧率
    resolution: "470", // 分辨率(1=720P 2=1080P…)
    customRatio: "452", // 开启自定义比例(bool)
    ratioW: "451", // 自定义比例宽
    ratioH: "450", // 自定义比例高
  };
  const norm = (v) => {
    ...
    return valueOrDefault(v, "");
  };
  const overrides = {};
  for (const [key, node] of Object.entries(KEY_TO_NODE)) {
    const v = norm(options[key]);
    if (v !== "") overrides[node] = v; // 空=未传,保留工作流默认
  }
  return nodeInfoList.map((node) => ({
    ...node,
    fieldValue: overrides[node.nodeId] !== undefined ? overrides[node.nodeId] : node.fieldValue,
  }));
}
...
async function buildAiAppVideoNodeInfo(product, options, context = {}) {
  ...
  const referenceVideo = String(
    options.referenceVideoUrl ||
      config.defaultReferenceVideo ||
      process.env.RUNNINGHUB_VIDEO_REFERENCE_VIDEO ||
      ""
  ).trim();
...
async function downloadAndExtractZip(zipUrl, context) {
  ...
  const videoEntry = entries.find(
    (e) =>
      !e.isDirectory &&
      (e.name.endsWith(".mp4") || e.name.endsWith(".mov") || e.name.endsWith(".webm"))
  );
...
async function generateRunningHubAiAppVideo(product, options, context = {}) {
  ...
  let videoResult = results.find((item) =>
    String(item.outputType || "")
      .toLowerCase()
      .includes("mp4")
  );
  if (!videoResult) {
...
module.exports = {
  ...
  buildVideoScript,
  buildVideoPrompt,
  buildAiAppVideoNodeInfo, // 导出供测试:离线断言节点映射(公网URL不触发上传)
  overrideVideoDefaults,
};
