...
async function runCopy(node, product, params) {
...
async function runImage(node, product, params) {
  const adapted = adaptProduct(product);
  const platform = adapted.platforms[0] || "抖音";
  const refs = await originalAbsPaths(product.id);
  ...
  const context = {
    ...
    productImages: refs.map((url) => ({ url })),
  };
...
async function runVideo(node, product, params) {
  const adapted = adaptProduct(product);
  const platform = adapted.platforms[0] || "抖音";
  const refs = await originalAbsPaths(product.id);
...
async function executeGenerateNode(node, product) {
  ...
  if (node.kind === "copy") return runCopy(node, product, params);
  if (node.kind === "image") return runImage(node, product, params);
  if (node.kind === "video") return runVideo(node, product, params);
