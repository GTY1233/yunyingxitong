...
async function runNodeJob(workflowId, nodeId, execOpts = {}) {
  try {
    ...
    let result;
    try {
      result = await executeGenerateNode(node, product, execOpts);
    } catch (e) {
...
async function act(workflowId, nodeId, actionType, opts = {}) {
  ...
  if (actionType === "execute") {
    ...
    // genParams:模特图/提示词/生成条数/参考视频/视频微调参数等(去掉 sync)
    const { sync, ...genParams } = opts;
    const job = runNodeJob(workflowId, nodeId, genParams); // 后台,不阻塞响应
    if (sync) await job;
