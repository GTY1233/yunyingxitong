...
// 对工作流中某节点施加动作。execute 会真正跑节点执行器(文案→真实生成),成功→已成功、失败→失败。
async function act(workflowId, nodeId, actionType) {
  ...
  if (actionType === "execute") {
    ...
    after = result.ok
      ? applyAction(wf.nodes, { type: "execute", nodeId })
      : applyAction(wf.nodes, { type: "fail", nodeId, error: result.error });
  } else {
    after = applyAction(wf.nodes, { type: actionType, nodeId });
  }
