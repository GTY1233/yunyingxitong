...
function imageRunningHubConfig() {
  return {
    ...
    mediaNodes: [
      { nodeId: "41", fieldName: "image", source: "modelImage", index: 0, description: "模特图", required: true },
      { nodeId: "79", fieldName: "image", source: "referenceImage", index: 0, description: "商品图", required: true },
      { nodeId: "81", fieldName: "image", source: "slot3", description: "补充图", required: false },
    ],
...
function videoRunningHubConfig() {
  return {
    ...
    mediaNodes: [
      { nodeId: "103", fieldName: "image", source: "referenceImage", index: 0, description: "参考图片", required: true },
      { nodeId: "161", fieldName: "video", source: "referenceVideo", description: "参考视频", required: true },
    ],
    nodeInfoDefaults: [
      ...
      { nodeId: "115", fieldName: "value", fieldValue: "1", description: "1速度快 2降低穿模 3特殊身材比例" },
      { nodeId: "116", fieldName: "value", fieldValue: "1.0000000000000002", description: "表情强度" },
      { nodeId: "132", fieldName: "value", fieldValue: "0.20000000000000004", description: "汝窑幅度" },
...
const DEFAULT_GENERATION_TEMPLATES = [
  {
    ...
    promptHint: "先脱光图一女生的全部衣服和帽子、鞋袜手套，再让图1的女人穿上图2款式的衣服，保持图一角色脸部、发型、姿态角度不变",
  },
  {
    id: "dy-image-detail",
    platform: "抖音",
  ...
  if (node.kind === "copy" && copyType) {
    const match = DEFAULT_GENERATION_TEMPLATES.find(
      (item) =>
        ...
        item.defaults?.copyType === copyType
    );
