...
function imageRunningHubConfig() {
  return {
    ...
    // 档位默认对齐工作流界面默认(1=保持原姿势/默认不改变/标准/直出);生成时可在对话框改档。
    // 姿势 1保持原姿势 2站立姿势;胸部 1默认不改变 2D/E饱满纯欲 3C标准自然 4A/B小巧清纯;
    // 腰臀比 1标准 2强化;输出方式 1直出 2ZIP(绕内容安审)。
    nodeInfoDefaults: [
      { nodeId: "105", fieldName: "select", fieldValue: "1", description: "姿势" },
      { nodeId: "120", fieldName: "select", fieldValue: "1", description: "胸部" },
      { nodeId: "114", fieldName: "select", fieldValue: "1", description: "腰臀比" },
...
function videoRunningHubConfig() {
  return {
    ...
    nodeInfoDefaults: [
      ...
      { nodeId: "265", fieldName: "value", fieldValue: "0.6", description: "表情强度" },
      { nodeId: "266", fieldName: "value", fieldValue: "0.27", description: "胸部抖动幅度" },
...
const DEFAULT_GENERATION_TEMPLATES = [
  ...
  },
  {
    ...
    // 调参默认全部在 videoRunningHubConfig.nodeInfoDefaults;这里只留展示用字段。
    defaults: {
      videoType: "商品展示视频",
      videoRatio: "9:16 竖版",
      videoDuration: "7 秒",
