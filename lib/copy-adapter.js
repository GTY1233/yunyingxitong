...
const PLATFORM_TONE = {
  抖音: "口语化、节奏快、穿搭分享感;用词中性安全(辣妹穿搭/纯欲风/ootd)",
  小红书: "种草感、生活方式、真实体验",
  淘宝: "信息完整、突出规格和性价比",
  拼多多: "实惠、实用、强调省钱",
  视频号: "亲切、可信、适合中老年用户",
...
function buildPrompt(product, options) {
  ...
  const complianceBlock = compliance.buildPromptRules(compliance.getRules(platform));
...
async function callCopyApi(product, options) {
  ...
  const versions = parseVersionsFromText(
    text,
  ...
  while (versions.length < versionCount) {
    versions.push(
      templateVariant(
        product,
        normalizeCopyType(options.copyType),
        options.platform,
        versions.length,
        options.extraPrompt
      )
